...
import com.example.model.UserQcDk;
import com.example.model.article;
import com.example.utils.StreamTool;

public class jsonUserQcDk {
	public static List<UserQcDk> getjsonlastarticles(String url)
			throws Exception {
		String path = url;
	...
	private static List<UserQcDk> parsejson(InputStream jsonStream)
			throws Exception {
		List<UserQcDk> list = new ArrayList<UserQcDk>();
		...
		for (int i = 0; i < jsonarray.length(); i++) {
			...
			String _addresspoint = jsonobject.getString("addresspoint");			
			String _memo = jsonobject.getString("memo");						
			String _adddate = jsonobject.getString("adddate");		
			
			list.add(new UserQcDk(_id, _userid, _companycode,
					_type, _address, _addresspoint, _memo,
					_adddate));
		}
		return list;
	}
